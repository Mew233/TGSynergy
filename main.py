...
def main():
    ...
    cell_dict = np.load('./data/CellLines_DepMap/CCLE_580_18281/census_706/cell_feature_cn_std.npy',
    ...
    IC = pd.read_csv('./data/IC50_loewe_copy.csv')
    ...
    args.num_feature = cell_dict['ACH-000046'].x.shape[1]
    #args.num_feature = 706
    ...
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            print("Training...")
            train_loss = train(model, train_loader, criterion, opt)
            fitlog.add_loss(train_loss.item(), name='Train MSE', step=epoch)

            print('Evaluating...')
            rmse, _, _, _ = validate(model, val_loader)
        ...
        train_rmse, train_MAE, train_r2, train_r = validate(model, train_loader)
        val_rmse, val_MAE, val_r2, val_r = validate(model, val_loader)
        test_rmse, test_MAE, test_r2, test_r = validate(model, test_loader)
